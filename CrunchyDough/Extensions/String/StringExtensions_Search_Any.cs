...
namespace CrunchyDough
{
    static public class StringExtensions_Search_Any
    {
        ...
        {
            if (start_index < item.Length)
            {
                int return_value = item.IndexOfAny(characters, start_index.BindAbove(0));
        ...
        static public bool TryFindLastIndexOfAny(this string item, out int index, char[] characters, int start_index)
        {
            if (start_index >= 0)
            {
                int return_value = item.LastIndexOfAny(characters, start_index.BindBelow(item.GetFinalIndex()));
