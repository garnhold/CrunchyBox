...
namespace CrunchyDough
{
    ...
    {
        private Stopwatch schedule_timer;
        private Scheduler<T> scheduler;

        public Schedule(long e, Process<T> p, TimeSource t)
        {
            schedule_timer = new Stopwatch(t).StartAndGet();
            scheduler = new Scheduler<T>(e, p, t);
        }

        public Schedule(long e, Process<T> p) : this(e, p, TimeSource_System.INSTANCE) { }
        ...
        public Schedule(Duration e, Process<T> p) : this(e, p, TimeSource_System.INSTANCE) { }

        public Schedule(Process<T> p) : this(p, TimeSource_System.INSTANCE) { }
